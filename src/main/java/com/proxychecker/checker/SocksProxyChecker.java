...
public class SocksProxyChecker implements ProxyChecker {
    ...
    private static final Logger log = LoggerFactory.getLogger(SocksProxyChecker.class);
    private static final ObjectMapper MAPPER = new ObjectMapper();
    private static final AtomicInteger FAILURE_LOG_COUNT = new AtomicInteger(0);
    private final OkHttpClient sharedClient;
    private final String testUrl;

    public SocksProxyChecker(OkHttpClient sharedClient, String testUrl) {
            ...
            client.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    if (FAILURE_LOG_COUNT.incrementAndGet() <= 5) {
                        log.warn("SOCKS proxy {} I/O error: {}", proxy.toUrl(), e.toString());
                    }
