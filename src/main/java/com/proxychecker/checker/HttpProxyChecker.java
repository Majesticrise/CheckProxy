    ...
    public CheckResult check(ProxyInfo proxy, long timeoutMillis, LocalIpDatabase localIpDatabase) {
        ...
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                boolean connectSupported = new ConnectTunnelTester().test(proxy, timeoutMillis);

                return CheckResult.success(proxy.toUrl(), elapsed, exitIp, location, connectSupported);
            }
        } catch (Exception e) {
